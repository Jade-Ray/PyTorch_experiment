...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b)) # pad is l,r,t,b,f,b
        _, Hp, Wp, _ = x.shape
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.downsample is not None:
            x_down = self.downsample(x, H, W) # B H/2*W/2 2*C
            Wh, Ww = (H + 1) // 2, (W + 1) //2
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0:
            x = F.pad(x, (0, self.patch_size[1] - W % self.patch_size[1])) # pad is (l,r,t,b)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3,
                 ...
                 out_indices=(0, 1, 2, 3), frozen_stages=-1, use_checkpoint=False, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        x = self.patch_embed(x) # B C Ph Pw
