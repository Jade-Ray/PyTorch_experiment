# %% [markdown]
# # 0️⃣Data Sourcing and Processing
# Train a translation model from scratch using Transformer
# ---
# - use `Multii30k` dataset from torchtext library that yield a pair of source-target raw sentences
...
# Create sourrce and target language tokenizer. Make sure to install the dependencies.
# pip install -U spacy
...
# %% [markdown]
# # 1️⃣Transformer Network
# The network consists of three parts
# ---
# - embedding layer that further augmented with positional encodings to provide position information
# - actual Transformer model
# - linear layer that give un-normlized probabilities for each token in the target language
...
# we need a subsequent word that will prevent model to look into the future words
# we also need masks to hide source and target padding tokens
def generate_square_subsequent_mask(sz):
    mask = (torch.triu(torch.ones((sz, sz), device=DEVICE)) == 1).transpose(0, 1) # anti-triu
...
# src and tgt language text transforms to convert rarw strings into tensors indices
text_transform = {}
for ln in [SRC_LANGUAGE, TGT_LANGUAGE]:
...
# actual function to translate input sentence into target languaage
